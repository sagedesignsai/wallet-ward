...
export function buildContentAgentContext(
  ...
  return {
    ...
    // generatePdf needs userId to set createdById on the ProjectFile record
    generatePdf: { organizationId, userId, projectId: options?.projectId },
    sendSlackNotification: shared,
    searchAuditLogs: shared,
    getProjects: shared,
    getRepositories: shared,
