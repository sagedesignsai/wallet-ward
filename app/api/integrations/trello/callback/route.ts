...
export async function GET(request: Request) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.redirect(
        new URL("/?error=trello_oauth_not_configured", process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000")
      )
    }
    ...
    const integration = await prisma.integration.create({
      data: {
        projectId,
        provider: "trello",
        name: trelloUser?.fullName ? `Trello (${trelloUser.fullName})` : "Trello",
    ...
    await storeEncryptedToken(integration.id, token, project.organizationId, "access")
