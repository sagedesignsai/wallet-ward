...
export async function GET(request: Request) {
  try {
    ...
    if (!clientId || !clientSecret) {
      return NextResponse.redirect(
        new URL("/?error=jira_oauth_not_configured", process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000")
      )
    }
    ...
    const resources = resourcesRes.ok
      ? ((await resourcesRes.json()) as Array<{ id: string; name: string; url: string }>)
      : []
    ...
    const integration = await prisma.integration.create({
      data: {
        projectId,
        provider: "jira",
        name: primaryResource?.name ? `Jira (${primaryResource.name})` : "Jira",
        scopes,
        tokenExpiresAt,
        metadata: {
          cloudId: primaryResource?.id ?? null,
          siteName: primaryResource?.name ?? null,
          siteUrl: primaryResource?.url ?? null,
